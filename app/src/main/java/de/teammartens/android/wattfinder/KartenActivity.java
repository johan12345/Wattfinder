...
*/
        ...
        if (LogWorker.isVERBOSE()) LogWorker.sendLog();
    ...
    protected void onResume() {
        ...
        if (LogWorker.isVERBOSE())
            LogWorker.d(LOG_TAG, "onResume " + mapReady + " " + AnimationWorker.getSTATE());
...
//        prepareSearch();
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (LogWorker.isVERBOSE())
            LogWorker.d(LOG_TAG, "onSaveInstanceState " + AnimationWorker.getSTATE());
        ...
        if (mapReady && mMap != null) {
            ...
            if (c != null) {
                outState.putFloat(zP_Latitude, new Float(c.latitude));
                outState.putFloat(zP_Longitude, new Float(c.longitude));
                outState.putString(zP_String, GeoWorks.getSuchString());
            } else {
                outState.putFloat(zP_Latitude, 0f);
                outState.putFloat(zP_Longitude, 0f);
            }
...
//Restoring the State
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        if (LogWorker.isVERBOSE()) LogWorker.d(LOG_TAG,
                "onRestoreInstanceState" + savedInstanceState.getFloat(zP_Latitude, 0));
        ...
        GeoWorks.Suchmarker(new LatLng(savedInstanceState.getDouble(zP_Latitude),
                        savedInstanceState.getDouble(zP_Longitude))
                , savedInstanceState.getString(zP_String, ""), false, false);

        /*}else{
            Float Lat = savedInstanceState.getFloat(zP_Latitude,0);
            Float Lng = savedInstanceState.getFloat(zP_Longitude,0);
        ...
        GeoWorks.setMapPosition(new LatLng(savedInstanceState.getDouble(sP_Latitude),
                        savedInstanceState.getDouble(sP_Longitude)),
    ...
    public void onBackPressed() {
        if (fragmentManager.getBackStackEntryCount() != 0 || AnimationWorker.getSTATE() > 0) {

// TODO: Hier muss unbedingt nachgearbeitet werden. Momentan funktioniert die Erkennung ob Map visible ist nur ab Android15

            fragmentManager.popBackStack();

            SupportMapFragment f = ((SupportMapFragment) getSupportFragmentManager().findFragmentById(
                    R.id.map));
            ...
            if (BackstackEXIT) {
                ...
                Toast.makeText(this, this.getString(R.string.Backstackexit),
    ...
    public void onMapReady(GoogleMap googleMap) {
        if (!mapReady) {
            ...
            v = getInstance().findViewById(R.id.fab_debug);
            if (v != null) v.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    LogWorker.setVERBOSE(!LogWorker.isVERBOSE());
                }
            });
            ...
            v.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    View V = v.getRootView().findViewById(R.id.fab_debug);
                    if (V != null) V.setVisibility(View.VISIBLE);
                    V = v.getRootView().findViewById(R.id.fab_search);
                ...
                public void onClick(View v) {
                    ...
                    if (checkPermissionLocation(MY_PERMISSIONS_REQUEST_LOCATION)) {
                        ...
                        if (GeoWorks.validLatLng(GeoWorks.getmyPosition())) {
                            ...
                            Toast.makeText(getInstance(), getString(R.string.novalidlocation),
                                    Toast.LENGTH_SHORT).show();
                            if (LogWorker.isVERBOSE())
                                LogWorker.d(LOG_TAG,
                                        "StandortButton: Kein gültiger Standort:" + GeoWorks.getmyPosition());
                        }
    ...
    public GoogleApiClient setupGoogleAPI() {
        ...
        PlayServiceStatus = GoogleApiAvailability.getInstance().isGooglePlayServicesAvailable(
                getBaseContext());
        ...
        if (PlayServiceStatus != ConnectionResult.SUCCESS) { // Google Play Services are not available
           ...
           /* try {
            ...
            LogWorker.e(LOG_TAG,
                    "PlayServices not connected:" + GoogleApiAvailability.getInstance().getErrorString(
                            PlayServiceStatus));

            Dialog dialog = GoogleApiAvailability.getInstance()
                    .getErrorDialog(this, PlayServiceStatus, CONNECTION_FAILURE_RESOLUTION_REQUEST,
                                ...
                                public void onCancel(DialogInterface dialog) {
                                    LogWorker.e(LOG_TAG,
                                            "onConnectionFailed: ConnectionResult.getErrorCode() = "
                                                    + PlayServiceStatus);
                                    CharSequence s = "Could not connect to Google API Client: Error " + PlayServiceStatus;
                                    Toast.makeText(KartenActivity.getInstance(), s,
                                            Toast.LENGTH_SHORT).show();
    ...
    private void prepareSearch() {
        ArrayAdapterSearchView searchView = findViewById(R.id.map_search);
        if (searchView == null && LogWorker.isVERBOSE())
            LogWorker.e("Building SearchView", "search was Null");
...
// TODO(Developer): Check error code and notify the user of error state and resolution.
    ...
    public static void setMapCenter() {
        ...
        LatLng mapPos = new LatLng(sharedPref.getFloat(sP_Latitude, GeoWorks.defaultLat),
                sharedPref.getFloat(sP_Longitude, GeoWorks.defaultLng));

        Float zoom = sharedPref.getFloat(sP_ZoomLevel, GeoWorks.DEFAULT_ZOOM);
        if (mapPos == null || !GeoWorks.validLatLng(mapPos)) {
            mapPos = GeoWorks.defaultLatLng;
        ...
        if (((System.currentTimeMillis() - TS) < TimestampValid) &&
                !mapPos.equals(GeoWorks.defaultLatLng) && zoom > GeoWorks.MAX_ZOOM) {
            GeoWorks.CUSTOM_MAPVIEW = true;

            LogWorker.d("SetMapCenter",
                    "Lat: " + mapPos.latitude + "Lng: " + mapPos.longitude + "Z: " + zoom + " geladen");
            GeoWorks.movemapPosition(mapPos, zoom, "mapReady");
        } else {
            GeoWorks.CUSTOM_MAPVIEW = false;
            if (GeoWorks.validLatLng(GeoWorks.getmyPosition())) {
                ...
                GeoWorks.movemapPosition(GeoWorks.defaultLatLng, GeoWorks.DEFAULT_ZOOM,
                        "mapReadyDefault");
            }
        }
    ...
    public static String layoutStyle() {
        ...
        if (v != null && v.getTag() != null && !v.getTag().toString().isEmpty()) {
            if (LogWorker.isVERBOSE())
                LogWorker.d(LOG_TAG, "LayoutStyle:-" + v.getTag().toString() + "-");
