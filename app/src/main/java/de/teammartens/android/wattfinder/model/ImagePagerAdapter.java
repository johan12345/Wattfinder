...
public class ImagePagerAdapter extends PagerAdapter implements View.OnClickListener {
    ...
    public ImagePagerAdapter(Context mContext, Bitmap[] drawables, Bitmap[] placeholders,
                             boolean HD) {
        this.mContext = mContext;
        this.mBitmaps = new Bitmap[drawables.length];
        for (int b = 0; b < drawables.length; b++) this.mBitmaps[b] = drawables[b];
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        View itemView = LayoutInflater.from(mContext).inflate(R.layout.imgpagerchild, container,
                false);

        if (LogWorker.isVERBOSE()) LogWorker.d(LOG_TAG,
                "Instantiate" + position + " Bitmap:" + (mBitmaps[position] != null ? mBitmaps[position].getByteCount() : "null"));
        this.iV[position] = itemView.findViewById(R.id.img_pager_item);
        ...
        if (mBitmaps[position] == null) {
            ...
            this.iV[position].setAlpha(0.0f);
            if (mPlaceholders != null) if (LogWorker.isVERBOSE()) LogWorker.d(LOG_TAG,
                    "Instantiate" + position + " setPlaceholder:" + (mPlaceholders[position] != null ? mPlaceholders[position].getByteCount() : "null"));
    ...
    private void setScaleType(int position) {
        ...
        if (HD) {
            ...
            if (LogWorker.isVERBOSE()) LogWorker.d(LOG_TAG,
                    "setScale" + position + " Bitmap:" + (mBitmaps[position] != null ? mBitmaps[position].getByteCount() + " w:" + mBitmaps[position].getWidth() + " h:" + mBitmaps[position].getHeight() : "null"));
