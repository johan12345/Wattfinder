...
public class Filter_plugs_Fragment extends Fragment {
    ...
    public void onStart() {
        super.onStart();
        ListView listView = this.getView().findViewById(R.id.filter_liste_plugs);
        ...
        final CheckBox cB = this.getView().findViewById(R.id.fPlugs_all);
        ...
        if (LogWorker.isVERBOSE()) LogWorker.d(LOG_TAG,
                " FilterListe " + LISTE + " enthält " + filterListe.size() + " Objekte.");
        ...
        if (LogWorker.isVERBOSE()) LogWorker.d(LOG_TAG,
                " FilterListeAdapter " + LISTE + " enthält " + dataAdapter.getCount() + " Objekte.");
    }
